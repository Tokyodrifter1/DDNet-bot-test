...
declare interface iMessage {
	team: number,
	client_id: number,
	utilisateur?: { InformationDuBot?: ClientInfo, InformationUtilisateur?: PlayerInfo },
	message: string
}
...
declare interface iKillMsg {
	killer_id: number,
	killer?: { InformationDuBot?: ClientInfo, InformationUtilisateur?: PlayerInfo },
	victim_id: number,
	victim?: { InformationDuBot?: ClientInfo, InformationUtilisateur?: PlayerInfo },
	weapon: number,
	special_mode: number
}
...
export declare interface Client {
	...
	on(event: 'connection_au_serveur_ddrace', listener: () => void): this;
	on(event: 'disconnect', listener: (reason: string) => void): this;

	on(event: 'emote', listener: (message: iEmoticon) => void): this;
	on(event: 'message_au_serveur', listener: (message: iMessage) => void): this;
					...
					if (chunk.sys) { 
						...
						if (chunk.msgid >= NETMSG_Sys.NETMSG_SNAP && chunk.msgid <= NETMSG_Sys.NETMSG_SNAPSINGLE) {
							...
							if (this.receivedSnaps == 2) {
								if (this.State != States.STATE_ONLINE)
									this.emit('connection_au_serveur_ddrace')
					...
					// })
						...
						if (chunk.msgid == NETMSG_Game.SV_EMOTICON) {
							...
							if (unpacked.client_id != -1) {
								unpacked.utilisateur = { 
									InformationDuBot:this.SnapshotUnpacker.getObjClientInfo(unpacked.client_id), 
									InformationUtilisateur: this.SnapshotUnpacker.getObjPlayerInfo(unpacked.client_id) 
								}
							}
							...
							this.emit("message_au_serveur", unpacked)
							...
							if (unpacked.victim_id != -1 && unpacked.victim_id < 64) {
								unpacked.victim = { InformationDuBot:this.SnapshotUnpacker.getObjClientInfo(unpacked.victim_id), InformationUtilisateur: this.SnapshotUnpacker.getObjPlayerInfo(unpacked.victim_id) }

							}
							if (unpacked.killer_id != -1 && unpacked.killer_id < 64)
								unpacked.killer = { InformationDuBot:this.SnapshotUnpacker.getObjClientInfo(unpacked.killer_id), InformationUtilisateur: this.SnapshotUnpacker.getObjPlayerInfo(unpacked.killer_id) }
